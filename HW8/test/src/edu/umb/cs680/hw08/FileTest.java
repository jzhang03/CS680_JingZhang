...
public class FileTest {
    ...
    Link x = new Link(home, "x", 0, Date, this.applications);
    Link y = new Link(code, "y", 0, Date, this.b);

    private String[] fileToStringArray(File f) {
        String[] fileInfo = {String.valueOf(f.isDirectory()), f.getName(), 
        		Integer.toString(f.getSize()), f.getCreationTime().toString(), 
        		f.getParent().getName(), String.valueOf(d.isFile()), String.valueOf(d.isLink())};
        return fileInfo;
    }

    @Test
    public void verifyFileEqualityA() {
        String[] expected = {"false", "a", "1", Date.toString(), "Applications", "true", 
        		"false"};
        File actual = this.a;
    ...
    public void verifyFileEqualityB() {
        String[] expected = {"false", "b", "1", Date.toString(), "Applications", "true", 
        		"false"};
        File actual = this.b;
        assertArrayEquals(expected, fileToStringArray(actual));
    }
    ...
    public void verifyFileEqualityC() {
        String[] expected = {"false", "c", "2", Date.toString(), "Home", "true", 
        		"false"};
        File actual = this.c;
        assertArrayEquals(expected, fileToStringArray(actual));
    }
    ...
    public void verifyFileEqualityD() {
        String[] expected = {"false", "d", "2", Date.toString(), "Home", "true", 
        		"false"};
        File actual = this.d;
        assertArrayEquals(expected, fileToStringArray(actual));
    }
    ...
    public void verifyFileEqualityE() {
        String[] expected = {"false", "e", "5", Date.toString(), "Code", "true", 
        		"false"};
        File actual = this.e;
        assertArrayEquals(expected, fileToStringArray(actual));
    }
    ...
    public void verifyFileEqualityF() {
        String[] expected = {"false", "f", "10", Date.toString(), "Code", "true", 
        		"false"};
        File actual = this.f;
        assertArrayEquals(expected, fileToStringArray(actual));
    }
